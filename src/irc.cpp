...
// Copyright (c) 2011-2012 EXchangeCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("EXchangeCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EXchangeCoinTEST0\r");
            Send(hSocket, "WHO #EXchangeCoinTEST0\r");
        } else {
            // randomly join #EXchangeCoin00-#EXchangeCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EXchangeCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EXchangeCoin%02d\r", channel_number).c_str());
        }
